# -*- coding: utf-8 -*-
"""[由 gen_ls_train.py 从 model_hvg2k.py 逐字生成，唯一改动 = 训练 loss 支持
label smoothing（**pergene_sup: 逐基因 eps**，由 gen_lspg.py 从 _ls 版生成）；label_smooth=0 时与原文件逐位相同。val 评估口径**未改**，仍是纯 CE，
...
def train_ours_hvg2k(train, val, V, iscale, seed, steps, *, batch=None, log_fn=print,
                     ...
                     label_smooth=0.0, ls_kernel="uniform_sup"):
    ...
    sched = BB._cosine_sched(opt, steps, CFG.WARMUP)
        ...
        with amp:
            ...
            if ls_u is None:
                ...
                if ls_e is None:
                    ...
                    loss = ((1.0 - ef_) * nll_ + ef_ * (-(uf_ * lp_).sum(-1))).mean()
